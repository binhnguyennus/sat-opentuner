...
then
    ...
    #
    ./solvers/Riss/classifier -no-xor -classify $cnfinstance -dataset="$wekatemp" -classifier="$model" -configInfo="$path/model/$version$drat-configurations.txt" -attrInfo="$attrtmp" -mem-lim=$classifierMemlimit -cpu-lim=$classifierTimeout -weka=weka-3.6.6.jar -predictor=predictor.jar $params -verb=1 > $classifierOutput
    code=$?
fi
...
then
	...
	./solvers/Riss/riss $cnfinstance -config=$selectedConfiguration
	# get the exit code
	ec=$?
else
	# use DRAT version
